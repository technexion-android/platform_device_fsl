...
build_imx_uboot()
{
	...
	if [ "`echo $2 | cut -d '-' -f2`" = "ddr4" ]; then
		cp ${UBOOT_OUT}/dts/upstream/src/arm64/freescale/imx8mn-ddr4-evk.dtb ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
		cp ${FSL_PROPRIETARY_PATH}/linux-firmware-imx/firmware/ddr/synopsys/ddr4* ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
	else
		cp ${UBOOT_OUT}/dts/upstream/src/arm64/freescale/imx8mn-evk.dtb  ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
