...
build_imx_uboot()
{
	...
	cp ${UBOOT_OUT}/arch/arm/dts/imx8mq-pico.dtb  ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
	...
	if [ `echo $2 | rev | cut -d '-' -f1 | rev` != "dual" ]; then
		...
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8M flash_pico_dual_bootloader || exit 1
