...
build_imx_uboot()
{
	...
	local _soc_type="imx8mq"
	local _uboot_dtb="${_soc_type}-pico-$(tr '[:upper:]' '[:lower:]' <<< ${EXPORT_BASEBOARD_NAME})_android.dtb"
	local _opt="-v"
	...
	if [ "`echo $2 | cut -d '-' -f2`" = "trusty" ]; then
		cp ${_opt} ${FSL_PROPRIETARY_PATH}/fsl-proprietary/uboot-firmware/imx8m/tee-imx8mq.bin ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/tee.bin
	...
	if [ `echo $2 | cut -d '-' -f2` = "ddr4" ]; then
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MQ  flash_ddr4_evk || exit 1
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MQ print_fit_hab_ddr4 || exit 1
	...
	local _mkcmd="make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MQ"
