...
TARGET_BOOTLOADER_BOARD_NAME := FLEX-IMX8MM

PRODUCT_MODEL := FLEX_IMX8MM
...
KERNEL_NAME := Image
BOARD_KERNEL_CMDLINE := init=/init androidboot.hwrotation=90 androidboot.console=ttymxc1 androidboot.hardware=freescale cma=$(CMASIZE) androidboot.primary_display=imx-drm firmware_class.path=/vendor/firmware transparent_hugepage=never quiet

# Default wificountrycode
BOARD_KERNEL_CMDLINE += androidboot.wificountrycode=TW

# Defaultly flex_imx8mm use QCA 1PJ wifi module, if use BCM 1MW module, set androidboot.wifivendor=bcm
...
BOARD_PREBUILT_DTBOIMAGE := out/target/product/flex_imx8mm/dtbo-imx8mm.img

TARGET_BOOTLOADER_CONFIG := flex-imx8mm_android_defconfig
TARGET_KERNEL_DEFCONFIG := tn_imx8_android_defconfig
TARGET_KERNEL_ADDITION_DEFCONF := android_addition_defconfig

ifeq ($(AUDIOHAT_ACTIVE),true)
TARGET_BOARD_DTS_CONFIG := imx8mm:imx8mm-flex-pi-ili9881c-voicehat.dtb
ifneq (,$(wildcard $(ADDITION_DRIVERS_PATH)/tfa98xx/snd-soc-tfa98xx.ko))
  BOARD_VENDOR_KERNEL_MODULES += \
  $(ADDITION_DRIVERS_PATH)/tfa98xx/snd-soc-tfa98xx.ko
endif
else
TARGET_BOARD_DTS_CONFIG := imx8mm:imx8mm-flex-pi-ili9881c.dtb
endif
