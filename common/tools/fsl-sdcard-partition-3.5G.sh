...
sfdisk_version=`${PWD}/device/fsl/common/tools/util-linux-2.26.2/sfdisk -v | awk '{print $4}' | awk -F '.' '{print $2}'`
...
# get total card size
seprate=100
total_size=`${PWD}/device/fsl/common/tools/util-linux-2.26.2/sfdisk -s ${node}`
# total_size=`expr ${total_size} / 1024`
if [[ ${total_size} -gt 3500 ]]; then
	total_size=3500
fi
...
${PWD}/device/fsl/common/tools/util-linux-2.26.2/sfdisk --force ${opt_unit}  ${node} << EOF
...
${PWD}/device/fsl/common/tools/util-linux-2.26.2/sfdisk --force ${opt_unit} ${node} -N1 << EOF
