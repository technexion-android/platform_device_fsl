...
if [ "${soc_name}" = "imx8qm" -o "${soc_name}" = "imx8qxp" -o "${soc_name}" = "imx8mq" -o "${soc_name}" = "imxpico_8m"  ]; then
...
function flash_android
{
    ...
    count_bootloader=`expr ${gpt_erase_size} - ${bootloader_offset}`
    echo "the bootloader file: ${bootloader_file}"
    echo "the bootloader partition size: ${count_bootloader}"
    echo "the bootloader offset: ${bootloader_offset}"
...
sleep 3
cd ../../../../
cd device/fsl/common/tools
echo "${PWD}"
sudo ./gpt_partition_move -d ${node} -s 8192
cd "${now_path}"
sync
