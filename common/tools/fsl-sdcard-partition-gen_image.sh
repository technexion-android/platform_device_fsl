...
if [ ${card_size} -ne 0 ] && [ ${card_size} -ne 9 ] && [ ${card_size} -ne 13 ] && [ ${card_size} -ne 28 ]; then
    help; exit 1;
fi
...
function flash_partition
{
    ...
    do
        if [ $? -eq 0 ]; then
            ...
            echo "flash_partition: ${img_name} ---> /dev/mapper/"${node_mapper}"p${num}"

            if [ "$(echo ${1} | grep "system")" != "" ] || [ "$(echo ${1} | grep "vendor")" != "" ] || \
                [ "$(echo ${1} | grep "product")" != "" ]; then
                simg2img ${image_directory}${img_name} /dev/mapper/"${node_mapper}"p${num}
            elif [ "$(echo ${1} | grep "super")" != "" ]; then
                if [ ${dont_generate_super} -eq 0 ]; then
                    simg2img /tmp/${img_name} /dev/mapper/"${node_mapper}"p${num}
                else
                    simg2img ${image_directory}${img_name} /dev/mapper/"${node_mapper}"p${num}
                fi
            else
                dd if=${image_directory}${img_name} of=/dev/mapper/"${node_mapper}"p${num} bs=10M conv=fsync
            fi
        fi
    done
}
