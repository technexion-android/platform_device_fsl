...
ifeq ($(PRODUCT_IMX_DUAL_BOOTLOADER),true)
  ...
  ifeq ($(TARGET_USE_DYNAMIC_PARTITIONS),true)
    ifeq ($(PRODUCT_IMX_CAR),true)
      BOARD_BPT_INPUT_FILES += $(CONFIG_REPO_PATH)/common/partition/device-partitions-28GB-ab-no-vb_super.bpt
      ADDITION_BPT_PARTITION = partition-table-13GB:$(CONFIG_REPO_PATH)/common/partition/device-partitions-13GB-ab-no-vb_super.bpt
    else
      ifeq ($(TARGET_INCLUDE_DTB_TO_VENDOR_BOOT),true)
        BOARD_BPT_INPUT_FILES += $(CONFIG_REPO_PATH)/common/partition/device-partitions-28GB-ab_super_gbl.bpt
        ADDITION_BPT_PARTITION = partition-table-13GB:$(CONFIG_REPO_PATH)/common/partition/device-partitions-13GB-ab_super_gbl.bpt \
                                 partition-table-dual:$(CONFIG_REPO_PATH)/common/partition/device-partitions-28GB-ab-dual-bootloader_super_gbl.bpt \
                                 partition-table-13GB-dual:$(CONFIG_REPO_PATH)/common/partition/device-partitions-13GB-ab-dual-bootloader_super_gbl.bpt
      else
